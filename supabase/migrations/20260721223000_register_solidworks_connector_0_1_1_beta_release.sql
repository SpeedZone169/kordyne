-- Register SolidWorks connector 0.1.1 beta release.
-- First customer-downloadable MSI with the tested publish, pull, comparison,
-- browser-login, responsive task-pane UI, and complete SolidWorks icon payload.
...
set
  ...
  security_metadata = coalesce(security_metadata, '{}'::jsonb) || jsonb_build_object(
    'downloadable', false,
    'replaced_by_version', '0.1.1-beta'
  ),
  updated_at = now()
where provider_key = 'solidworks'
...
with upserted_release as (
  ...
  values (
    ...
    'solidworks/Kordyne-SolidWorks-Connector-0.1.1-beta.msi',
    'Kordyne-SolidWorks-Connector-0.1.1-beta.msi',
    'application/x-msi',
    1368064,
    '2E8C7B9F306C670848477DEE4787BD99E8B8A50A35393DAAB16F396A33124C2E',
    'SolidWorks connector beta adds the tested controlled publish, native-or-STEP pull, aligned visual comparison, browser login, responsive task-pane UI, production endpoint restrictions, and the complete SolidWorks task-pane icon set.',
    true,
    jsonb_build_object(
      'downloadable', true,
