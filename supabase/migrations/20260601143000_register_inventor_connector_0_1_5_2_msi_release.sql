-- Register Inventor connector 0.1.5.2 beta release.
-- Refreshes the Inventor ribbon icon from the designer source, avoids stale
-- Inventor command icon caching, and keeps the final 0.1.5.x library polish.
...
set
  ...
  security_metadata = coalesce(security_metadata, '{}'::jsonb) || jsonb_build_object(
    'downloadable', false,
    'replaced_by_version', '0.1.5.2-beta'
  ),
  updated_at = now()
...
with upserted_release as (
  ...
  values (
    ...
    'inventor/Kordyne-Inventor-Connector-0.1.5.2-beta.msi',
    'Kordyne-Inventor-Connector-0.1.5.2-beta.msi',
    'application/x-msi',
    1478656,
    '1BBDB78213D5A640D6848425066C2AFC78D1241530CFB1E5540DD99249F90BAC',
    'Inventor connector beta refreshes the Kordyne ribbon icon with a crisper designer-derived asset, forces Inventor to load the new command icon instead of a cached older bitmap, and preserves the final library and publish polish.',
    true,
    jsonb_build_object(
      'downloadable', true,
