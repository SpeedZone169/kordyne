-- Register Inventor connector 0.1.4.3 beta release.
-- This patch fixes narrow-panel layout issues, active-context badge sizing,
-- and expanded revision action rows.
...
set
  ...
  security_metadata = coalesce(security_metadata, '{}'::jsonb) || jsonb_build_object(
    'downloadable', false,
    'replaced_by_version', '0.1.4.3-beta'
  ),
  updated_at = now()
...
with upserted_release as (
  ...
  values (
    ...
    'inventor/Kordyne-Inventor-Connector-0.1.4.3-beta.msi',
    'Kordyne-Inventor-Connector-0.1.4.3-beta.msi',
    'application/x-msi',
    1474560,
    'D2082A0FED3123A4D74B2B7DE67F1492C1F8468874ECC46A19636377339512C4',
    'Inventor connector beta refresh fixes narrow panel clipping, restores compact active-context status badges, strengthens scroll reset behavior, and keeps Pull, Compare, and Select actions visible on expanded revision rows.',
    true,
    jsonb_build_object(
      'downloadable', true,
