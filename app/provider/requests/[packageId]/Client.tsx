...
import ProviderPackageFilesViewer from "./ProviderPackageFilesViewer";
...
export default function Client({ data }: Props) {
  ...
  const previewableFilesCount = useMemo(
    () => data.files.filter((file) => file.previewKind !== "other").length,
    [data.files],
  );
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Review shared package files directly in-page. PDF, image, STL,
                  and STEP can be checked without leaving the request.
                </p>
              </div>
            ...
            <div className="mt-6">
              <ProviderPackageFilesViewer files={data.files} />
            </div>
          </div>
