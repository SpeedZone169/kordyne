...
function isNativeInventorFile(fileName: string) {
  return fileName.endsWith(".ipt") || fileName.endsWith(".iam");
}
...
async function signFiles(
  ...
  for (const file of files) {
    ...
    const filename = asString(file.file_name) || "inventor-file";
...
export async function POST(request: Request) {
  try {
    ...
    const nativeFiles = (files ?? []).filter((file) =>
      isNativeInventorFile(getLowerFileName(file.file_name)),
    ...
    if (signedNativeFiles.length === 0 && signedStepFiles.length === 0) {
      return NextResponse.json(
        {
          ok: false,
          error: "No native Inventor or STEP files found for this part.",
        },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json({
      ...
      message: "Inventor pull package is ready.",
    });
  } catch (error) {
    return NextResponse.json(
      {
