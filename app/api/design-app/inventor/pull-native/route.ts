...
function isNativeInventorFile(fileName: string) {
  const lower = fileName.toLowerCase();
  return lower.endsWith(".ipt") || lower.endsWith(".iam");
}

function isAssemblyFile(fileName: string) {
  return fileName.toLowerCase().endsWith(".iam");
}
...
export async function POST(request: Request) {
  try {
    ...
    const nativeFiles = (files ?? []).filter((file) =>
      isNativeInventorFile(String(file.file_name ?? "")),
    );

    if (nativeFiles.length === 0) {
      return NextResponse.json(
        { ok: false, error: "No native Inventor IPT/IAM files found for this part." },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json({
      ...
      files: signedFiles,
      message: "Native Inventor files are ready for pull.",
    });
  } catch (error) {
    return NextResponse.json(
      {
