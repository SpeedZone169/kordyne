...
export async function POST(request: Request) {
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Inventor connector is not configured for this organization.",
        },
        { status: 403 },
      );
    }
    ...
    for (const file of files) {
      ...
      if (role !== "step") {
        return NextResponse.json(
          { ok: false, error: "Only STEP files can be published from this flow." },
          { status: 400 },
        );
      }
    }
    ...
    if (publishMode === "new_revision") {
      ...
      if (!sourcePartId) {
        return NextResponse.json(
          { ok: false, error: "part_id is required for new revision publish." },
          { status: 400 },
        );
      }
      ...
      if (!sourcePart?.id) {
        return NextResponse.json(
          { ok: false, error: "Target part not found." },
          { status: 404 },
        );
      }
      ...
      if (partUpdateError) {
        return NextResponse.json(
          { ok: false, error: partUpdateError.message },
          { status: 500 },
        );
      }
    } else {
    ...
    if (createdPartError) {
      return NextResponse.json(
        { ok: false, error: createdPartError.message },
        { status: 500 },
      );
    }
    ...
    if (!createdPart?.id || !createdPart.part_family_id) {
      return NextResponse.json(
        { ok: false, error: "Created part could not be loaded." },
        { status: 500 },
      );
    }
    ...
    for (const file of files) {
      ...
      if (partFileError) {
        return NextResponse.json(
          { ok: false, error: partFileError.message },
          { status: 500 },
        );
      }
    }
    ...
    if (syncRunError) {
      return NextResponse.json(
        { ok: false, error: syncRunError.message },
        { status: 500 },
      );
    }
