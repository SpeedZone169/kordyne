  ...
  try {
    ...
    if (!connector?.id) {
      return NextResponse.json(
        {
          ok: false,
          error: "SolidWorks connector is not configured for this organization.",
        },
        { status: 403 },
      );
    }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Only STEP, STL, native SolidWorks, properties text, and preview thumbnail files can be published from this flow.",
          },
          { status: 400 },
        );
      }
        ...
        const { data: insertedThumbnailFile, error: thumbnailPartFileError } =
          await admin
            ...
            .insert({
              part_id: createdPart.id,
              user_id: ctx.user.id,
              file_name: metadataThumbnailFileName || "SolidWorks preview thumbnail.png",
              file_type: "image/png",
    ...
    const partSourceMetadata = {
      source: "solidworks_addin_publish",
    ...
    const responsePayload = {
      ...
      thumbnail_storage_path: thumbnailStoragePath,
      idempotency_key: idempotencyKey,
      message: "SolidWorks publish completed successfully.",
    };
