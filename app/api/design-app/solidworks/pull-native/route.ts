...
function isNativeSolidWorksFile(fileName: string) {
  ...
  return (
    ...
    lower.endsWith(".slddrw")
  );
}

function isAssemblyFile(fileName: string) {
  return fileName.toLowerCase().endsWith(".sldasm");
}
...
export async function POST(request: Request) {
  try {
    ...
    const nativeFiles = (files ?? []).filter((file) =>
      isNativeSolidWorksFile(String(file.file_name ?? "")),
    );
    ...
    if (nativeFiles.length === 0) {
      return NextResponse.json(
        {
          ok: false,
          error: "No native SolidWorks SLDPRT/SLDASM/SLDDRW files found for this part.",
        },
        { status: 404 },
      );
    }
    ...
    for (const file of nativeFiles) {
      ...
      signedFiles.push({
        ...
        filename: String(file.file_name ?? "solidworks-file"),
    ...
    return NextResponse.json({
      ...
      files: signedFiles,
      message: "Native SolidWorks files are ready for pull.",
    });
  } catch (error) {
    return NextResponse.json(
      {
