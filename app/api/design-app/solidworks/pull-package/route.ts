...
function isNativeSolidWorksFile(fileName: string) {
  return (
    fileName.endsWith(".sldprt") ||
    fileName.endsWith(".sldasm") ||
    fileName.endsWith(".slddrw")
  );
}
...
function isAssemblyFile(fileName: string) {
  return fileName.endsWith(".sldasm");
}
...
async function signFiles(
  ...
  for (const file of files) {
    ...
    const filename = asString(file.file_name) || "solidworks-file";
    ...
    signedFiles.push({
      ...
      is_primary: false,
      is_assembly: isAssemblyFile(filename.toLowerCase()),
    });
  }
...
export async function POST(request: Request) {
  try {
    ...
    const nativeFiles = (files ?? []).filter((file) =>
      isNativeSolidWorksFile(getLowerFileName(file.file_name)),
    ...
    if (signedNativeFiles.length === 0 && signedStepFiles.length === 0) {
      return NextResponse.json(
        {
          ok: false,
          error: "No native SolidWorks or STEP files found for this part.",
        },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json({
      ...
      message: "SolidWorks pull package is ready.",
    });
  } catch (error) {
    return NextResponse.json(
      {
