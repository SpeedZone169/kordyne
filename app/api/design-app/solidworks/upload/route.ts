...
function uploadMessageForRole(role: string) {
  if (role === "native") return "SolidWorks native file uploaded successfully.";
  if (role === "stl") return "SolidWorks STL viewer file uploaded successfully.";
  if (role === "thumbnail") return "SolidWorks preview thumbnail uploaded successfully.";
  if (role === "properties") return "SolidWorks properties text file uploaded successfully.";
  return "SolidWorks STEP file uploaded successfully.";
}

function invalidExtensionMessageForRole(role: string) {
  if (role === "native") {
    return "Only SLDPRT, SLDASM and SLDDRW files are allowed for native SolidWorks upload.";
  }

  if (role === "stl") {
    return "Only STL files are allowed for SolidWorks viewer upload.";
  }
  ...
  if (role === "properties") {
    return "Only TXT files are allowed for SolidWorks properties upload.";
  }
